...
#We output a table and a figure; both compare
...
number =100# number=a few neonates when testing the script; but usually=100 in actual calculations.
...
####################Preprocessing#########################
dose_input = 'csv_files/front_input_825_dose.csv'#loads the chosen dose cycles; other input files of the same structure can be used.
#table = outputMultiregimen(dose_input, 'csv_files/front_tableOutput_825.csv', miley)
table = read.csv('csv_files/front_tableOutput_825.csv')# used as initialization;
table[,"flagged"] = 1 #initialize flagged parameter to 1 for all doses
peak1 = table[,"c0.1"]; peak2 = table[,"c24.1"]; peak3 = table[,"c48.1"]; #define variables peak1,2,3 to be the peak of concentration
trough1 = table[,"c23.99"];trough2 = table[,"c47.99"];trough3=table[,"c71.99"]; #define variables trough1,2,3 to be the lowest point of concentration
peak = as.double(peak1)+as.double(peak2)+as.double(peak3); trough = as.double(trough1)+as.double(trough2) 
flagged = which(peak < 36 & trough < 4) #find the doses whose peak is less than 36 and trough is less than 4
table[flagged,"flagged"] = 0 #set those doses to 0
l = nrow(table)
E40 = as.double(table[1:l, "E40"])#E_F for the prototypical GA 40 wk female neonate and all dose cycles
rank = order(E40)# order the rows by E_F
...
dose1 = doses[1,]; dose2 = doses[2,]; dose3 = doses[3,]; #get the doses from the dosing cycle
...
i_sim = boncoeff[1]*dose1+ boncoeff[2]*dose2+boncoeff[3]*dose3 #B for the Hill (i.e. Fisk) function 1/[1+(1/B)^b], where the shape parameter b=boncoeff[5]
...
E40_func <- function(B){
  pfisk(B,1,boncoeff[4]) #scale parameter set equal to 1 without essential loss of generality. Boncoeff[5]=shape parameter=sigmoidicity index
}
...
write.csv(table, 'csv_files/tableOutput.csv')
