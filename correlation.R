...
####################Preprocessing#########################
dose_input = 'miley_input_56_doses.csv'#loads the chosen dose cycles; other input files of the same structure can be used.
...
rrho <- function(coeff) {
  ...
  x = a*dose1+b*dose2+c*dose3+d*dose4
  val = pfisk(x, 1, e)
  1-cor(val,E40)
}
#We used simulated annealing to optimize parameters of the Hill function
global.min = 0; tol = 0.0001; lower = rep(0,5); upper = c(1,1,1,1,20)
out <- GenSA(par = c(0.5,0.5,0.5,0.5,10), lower = lower, upper = upper, fn = rrho,
...
i_sim = boncoeff[1]*dose1+ boncoeff[2]*dose2+boncoeff[3]*dose3+boncoeff[4]*dose4 #B for the Hill (i.e. Fisk) function 1/[1+(1/B)^b], where the shape parameter b=boncoeff[5]
E40_p = pfisk(i_sim,1,boncoeff[5])  #the predicted probability E40=E_F

#This is the function that is used to predict the probability given the value B.
E40_func <- function(B){
  pfisk(B,1,boncoeff[5]) #scale parameter set equal to 1 without essential loss of generality. Boncoeff[5]=shape parameter=sigmoidicity index
}
...
plot(i_sim[rank],E40[rank], type = 'p', xlab = 'B=b1*d1+b2*d2+b3*d3+b4*d4', ylab = 'E40')
...
table[,'B=b1d1+b2d2+b3d3+b4d4'] = i_sim #add B to the table. Comment out following line to retain 'miley_output_50_doses.csv'
