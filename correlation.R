...
#We output a table ("miley_output_50_doses.csv" and a figure; both compare
...
####################Preprocessing#########################
dose_input = 'miley_input_50doses.csv'#loads the chosen dose cycles
table = read.csv('miley_output_50_doses.csv')# used as initialization; will be overwritten by script below
l = nrow(table)
...
global.min = 0; tol = 0.001; lower = rep(0, 3); upper = rep(100, 3)#BON: if you can hold boncoeff[3] constant at a=1
## during simulated annealing, do that and simplify the following. However the following works
out <- GenSA(par = c(25,25,25), lower = lower, upper = upper, fn = rrho,
...
i_sim = boncoeff[1]*dose1+ boncoeff[2]*dose2 #B for the Hill (i.e. Fisk) function 1/[1+(1/B)^b], where the shape parameter b=boncoeff[4]
E40_p = pfisk(i_sim,1,boncoeff[3])  #the predicted probability E40=E_F

#for the protoypical neonate, plot the virtual data points and the optimized Hill function
plot(i_sim,E40, type = 'p', xlab = 'B=b1*d1+b2*d2', ylab = 'E40')# to properly renormalize, x-axis labels must be divided by 80.5
legend("bottomright", legend = c('E40','E40_predicted'), col = c('black','red'), lty=1)
lines(i_sim,E40_p,col = 'red')
...
write.csv(table, 'miley_output_50_doses.csv')
